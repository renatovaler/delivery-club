...
import { DeliveryGroup, SubscriptionSummary, DashboardData } from './dashboard-types';
import { addDays, differenceInCalendarWeeks, format } from 'date-fns';

// Helpers para lidar com o fuso horário de São Paulo
export const formatInSaoPaulo = (date: Date, options: Intl.DateTimeFormatOptions): string => {
...
export const isSameSaoPauloDay = (date1: Date, date2: Date): boolean => {
...
function calculateUpcomingDeliveries(
  subscriptions: Subscription[],
  allItems: SubscriptionItem[],
): DeliveryGroup[] {
  ...
  const subscriptionsById = subscriptions.reduce<Record<string, Subscription>>((acc, sub) => {
    acc[sub.id] = sub;
    return acc;
  }, {});
...
export async function loadDashboardData(currentUser: User): Promise<DashboardData> {
  if (!currentUser) {
    return {
      subscriptions: [],
      upcomingDeliveries: [],
  ...
  // Usando Array.from em vez de spread operator
  const uniqueTeamIds = new Set(userSubscriptions.map(s => s.team_id));
  ...
  const allItemsPromises = userSubscriptions.map(s => 
    SubscriptionItemAPI.filter({ subscription_id: s.id })
  );
  const allItemsArrays = await Promise.all(allItemsPromises);
  const allItems = allItemsArrays.reduce((acc, items) => acc.concat(items), []);
  ...
  const upcomingDeliveries = calculateUpcomingDeliveries(userSubscriptions, allItems);
